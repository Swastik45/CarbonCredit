...
export default function ConfirmPage() {
  ...
  useEffect(() => {
    ...
    if (success === 'true') {
      ...
      // Handle direct token confirmation
      const confirmEmail = async () => {
        ...
        try {
          const response = await fetch('/api/auth/confirm?' + new URLSearchParams({
            token_hash,
            type
          }));

          if (response.redirected) {
            // Handle redirect from API route
            ...
            if (success === 'true') {
              ...
              setStatus('error');
              setMessage(error === 'invalid_link' ? 'Invalid confirmation link' :
                        error === 'failed' ? 'Confirmation failed. Please try again.' : 'An error occurred');
            }
          } else {
            setStatus('error');
          ...
          {status === 'error' && (
            <>
              ...
              <div className="space-y-3">
                ...
                <Link
                  href="/login"
                  className="block text-emerald-600 font-bold hover:text-emerald-700"
                >
                  Back to Login
                </Link>
              </div>
